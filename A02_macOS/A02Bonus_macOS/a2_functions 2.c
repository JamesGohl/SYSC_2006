...
friend_t *create_friend(user_t *users, const char *username)
{
    ...
    //friend_t *found_friend = find_friend(user, username);
...
void manage_friends(user_t *user, user_t *users)
{
    ...
    while (true)
    {
        ...
        if (user_choice == 1)
        {
            ...
            if (new_friend != NULL && old_friend == NULL)
            {
                add_friend(user, users, username);
               // friend_t *found_friend = find_friend(user, username);
               // found_friend->posts = &(new_friend->posts);
               // printf("user->friends->posts %s\n", (*(found_friend->posts))->content);
...
void display_friend_posts(user_t *user)
{
    ...
    else
    {
        printf("gorrila\n");
        printf("username %s\n", friend->username);
        printf("TESTESTuser->friends->posts %s\n", (*(friend->posts))->content);
