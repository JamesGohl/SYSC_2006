...
float get_input_f(void)
{
	...
	do
	{
		...
		if (return_value <= 0)
		{
			printf("The input is invalud please enter a number greator than zero\n");
		}
...
void init_milestone(milestone_t *milestone_array, unsigned short int num_milestones)
{
	...
	for (int i = 0; i < num_milestones; i++)
	{
		...
		(milestone_array + i)->activity_capacity = num_activites;
		activity_t *activity_array_ptr = malloc(num_activites * sizeof(activity_t));
		assert(activity_array_ptr != NULL);
		...
		for (int k = 0; k < num_activites; k++)
		{
			...
			else
			{
				...
				(activity_array_ptr + k)->id = activity_id_to_check;
				strcpy((activity_array_ptr + k)->name, "namee");
				(activity_array_ptr + k)->planned_duration =3;
				printf("banana");
				init_activity(activity_array_ptr + k);
			}
		}
		activity_id_list_position++;
	}
...
void init_activity(activity_t *activity)
{
	...
	//printf(", (activity)->name); %s",(activity)->name);
	//scanf("%99s", (activity)->name);
	//printf(" (activity)->plann %hu", activity->planned_duration);
	//printf("Please enter the planned durration (hours) of the activity %s, ID: %hu: ", activity->name, activity->id);
	//unsigned short int planned_durration = get_input_usi();
	//printf(" 2 succes until now");
	//(*activity).planned_duration += planned_durration;
	//("TESTPlease enter the name of activity with id: %hu\n", (activity)->id);
	//printf(", (activity)->name); %s",(activity)->name);
	///scanf("%99s", (activity)->name);
	//printf(" 1 succes until now");
