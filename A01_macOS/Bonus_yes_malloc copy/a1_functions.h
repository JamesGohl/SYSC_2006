...
#endif
/*
My solution for checking ID's was already relatiively easy, so not much change was needed.
Previously I had to mannualy initialize the activities that were not needed to fill the array.
Then when I display the activities I had to check and not display those manually initialized arrays.
However when using dynamic memory, the array size of the actiities and milestones where only as large as needed so no manual inizatliaton was needed.
I still kept the check_unique_id function which takes an array of ids and an id and checks if the id aready exists. However,
the id array was now dynamically allocated based on the total number of activites. To get the number of activites though, I needed to
ask for the number of all activities in each milestione at that begining rather than after each seperate milestone initialiation.
*/
...
_Bool check_unique_id(unsigned short int id_to_check, const unsigned short int *activity_id_list, int check_up_to); // checks if an activity id already existis in a milestone
