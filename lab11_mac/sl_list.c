...
{
    intnode_t *new_node;
    intnode_t *current, *previous;
    int i;
    for (current = head, previous = NULL, i = 0; current != NULL && i < index; previous = current, current = current->next, i++)
    {
    }
    // insert at end
    if (i == index)
    {
        new_node = intnode_construct(elem, current);
        if (previous == NULL)
            return new_node;
